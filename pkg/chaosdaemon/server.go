...
func (s *DaemonServer) getLoggerFromContext(ctx context.Context) logr.Logger {
    return s.rootLogger.WithValues("context", ctx)
}

func (s *DaemonServer) RecoverTimeOffset(ctx context.Context, req *pb.TimeRequest) (*emptypb.Empty, error) {
    s.rootLogger.Info("RecoverTimeOffset called", "container", req.ContainerId)
    return &emptypb.Empty{}, nil
}

// This is a stub to fix the compilation error.
// You should implement it properly if your proto expects it.
func (s *DaemonServer) SetTimeOffset(context.Context, *pb.TimeRequest) (*emptypb.Empty, error) {
    s.rootLogger.Info("SetTimeOffset called (stub)")
    return &emptypb.Empty{}, nil
}
...
func (s *Server) Start() error {
    ...
    eg.Go(func() error {
        s.logger.Info("Starting grpc endpoint", "address", grpcBindAddr)
